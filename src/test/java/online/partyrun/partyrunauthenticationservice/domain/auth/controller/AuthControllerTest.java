...
class AuthControllerTest extends RestDocumentTest {
    ...
    class idToken이_주어졌을_때 {
        IdTokenRequest request = new IdTokenRequest("idSample");
        final JwtToken expected = new JwtToken("3Y2NiZmIwMDg5NzQ1ZDA1ZiIs.eyJpZCI6IjY0ODIiZmIwMDg5NzQ1ZD4NTc3Y2NiZmIwMDg5NzQ1ZDA1ZiIsImV4cCI6MTY4NzMwNzY1NH0.4Jka7hW3NWESLOOmNj-_XK-Yf9TKoxW42Vq_aMO6jB_uIp6-mpdamJ43F7ADu57RTkbCnMJ8b06TA5kLEIlJqQ",
        ...
        class idToken이_잘못된_형식이라면 {
            ...
            @DisplayName("예외를 반환한다")
            void returnException() throws Exception {
                given(authService.authorize(anyString())).willThrow(new IllegalIdTokenException("idToken"));
                ResultActions actions =
                        mockMvc.perform(
                                post("/auth")
                                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Nested
    @DisplayNameGeneration(DisplayNameGenerator.ReplaceUnderscores.class)
    class refreshToken이_주어졌을_떄 {
        String refreshToken = "eyJhbGciOiJIUzUxMiJ9.eyJpZCI6IjY0ODI4NTc3Y2NiZmIwMDg5NzQ1ZDA1ZiIsImV4cCI6MTY4NzMwNzY1NH0.4Jka7hW3NWESLOOmNj-_XK-Yf9TKoxW42Vq_aMO6jB_uIp6-mpdamJ43F7ADu57RTkbCnMJ8b06TA5kLEIlJqQ";
        final AccessTokenResponse expected = new AccessTokenResponse("Y2NiZmIwMDg5IUzUxMiJ9.eyJpZCI6IjY0ODI4NTc3Y2NiZmIwMDg5NzQ1ZDA1ZiIsImV4cCI6MTY4NzMwNzY1NH02Vq_aMO6jB_uIp6-mpda8b06TA5kLEIlJqQ");
        @Test
        @DisplayName("access token을 재발급해준다")
        void returnAccessToken() throws Exception {
            given(authService.refreshAccessToken(anyString())).willReturn(expected);
            ResultActions actions =
                    mockMvc.perform(
                            post("/auth/access")
                                    .contentType(MediaType.APPLICATION_JSON)
        ...
        class refreshToken이_잘못된_형식이라면 {
            ...
            @DisplayName("예외를 반환한다")
            void returnException() throws Exception {
                given(authService.refreshAccessToken(anyString())).willThrow(new IllegalIdTokenException("idToken"));
                ResultActions actions =
                        mockMvc.perform(
                                post("/auth/access")
                                        .contentType(MediaType.APPLICATION_JSON)
